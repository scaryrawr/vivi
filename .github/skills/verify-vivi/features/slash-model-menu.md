...
- Copilot default, authenticated Copilot models, and OMLX model/reasoning
  combinations share one picker with a current-selection marker.
- Hosted and OMLX rows show context, output, vision, and reasoning metadata
  when known.
...
- The authenticated Copilot account must expose `gpt-5.6-luna` or the explicit
  `VIVI_VALIDATION_MODEL`.
