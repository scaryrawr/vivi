...
- `reasoning-levels` reports `off,low,medium,high,xhigh` for every local model.
...
- Run `vivi models`.
- Run `vivi chat --model omlx/<model-id> --reasoning high`.
...
- Run `./zig-out/bin/vivi models` and retain stdout showing the selected test
  model's `context=`, `output=`, `vision=`, and
  `reasoning=off,low,medium,high,xhigh` fields.
- Start
  `./zig-out/bin/vivi chat --model omlx/<model-id> --reasoning high` in a
  fresh `script` PTY, submit a deterministic prompt, and retain the terminal
  log.
