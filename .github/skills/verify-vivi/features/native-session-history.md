...
two project sections, resuming a saved Vivi session, and selecting both
resulting duplicate-workspace rows. Attempt a second click while resume is
pending, close and reopen the window through the Dock, then quit. Retain the
video and representative screenshots alongside the exact Xcode and repository
check output.
