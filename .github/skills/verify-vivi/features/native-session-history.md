...
- Live conversation rows appear first in roster order, followed by a nested
  History disclosure containing resumable sessions saved for that project.
...
`~/.vivi/copilot/`. Launch it again from another workspace, expand History
under each project in the sidebar, and select a persisted session. Repeat with another live
...
History under two projects, resuming a saved Vivi session, and selecting both
resulting duplicate-workspace rows. Verify one live row is highlighted at a
time and selection does not reorder either project's roster. Attempt a second
click while resume is pending, close and reopen the window through the Dock,
then quit. Retain the video and representative screenshots alongside the exact
