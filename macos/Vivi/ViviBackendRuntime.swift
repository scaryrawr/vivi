...
final class NativeChatStore: ObservableObject {
  ...
  func refreshModels() {
    guard modelState == .ready, sessionState == .ready, commandExecution == nil else { return }
  ...
  private func switchModel(_ selection: ModelSelection) {
    guard modelState == .ready, sessionState == .ready, commandExecution == nil,
...
enum NativeEventDecoder {
  static func decode(
    ...
    guard event.reserved == 0, event.allow_freeform <= 1,
    ...
    case VIVI_BACKEND_EVENT_USER_INPUT_REQUEST:
      guard event.content_kind == VIVI_BACKEND_CONTENT_USER_INPUT_REQUEST,
        ...
        event.default_saved == 0, event.cleanup_failed == 0,
        event.skipped_invalid_shards == 0, zero(event.content),
        hasNeutralConversationMetadata
      else { throw NativeEventDecodingError.malformed }
    ...
    case VIVI_BACKEND_EVENT_COMMAND_CATALOG:
      guard event.content_kind == VIVI_BACKEND_CONTENT_COMMAND_CATALOG,
        ...
        event.default_saved == 0, event.cleanup_failed == 0,
        event.skipped_invalid_shards == 0, event.allow_freeform == 0,
